...
async function checkDatabase(req: MedusaRequest): Promise<ServiceCheck> {
...
function configReport(): Array<{ name: string; set: boolean; required: boolean }> {
  const required = [
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const hasDegraded = checks.some((c) => c.status === "degraded" || c.status === "not_configured")
